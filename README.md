...
## Kafka default service (optional)

BukkitSpring can expose a shared Kafka service to other plugins. Enable it in `config.yml`:
...
Then inject `KafkaService` and build your client with the default properties:

```java
import com.cuzz.bukkitspring.api.kafka.KafkaService;
import org.apache.kafka.clients.producer.KafkaProducer;
import org.apache.kafka.common.serialization.StringSerializer;

import java.util.Map;

KafkaProducer<String, String> producer = kafkaService.createProducer(Map.of(
        "key.serializer", StringSerializer.class.getName(),
        "value.serializer", StringSerializer.class.getName()
));
```
The Kafka executor prefers JDK virtual threads when available (JDK 21+), and falls back to a cached thread pool on older runtimes.
You can also access the shared instance via `BukkitSpring.getKafkaService()`.

## Redis default service (optional)
...
Then inject `RedisService` and use the shared Jedis client:

```java
import com.cuzz.bukkitspring.api.redis.RedisService;
import redis.clients.jedis.JedisPooled;

JedisPooled jedis = redisService.client();
jedis.set("example", "value");
```

The Redis executor prefers JDK virtual threads when available (JDK 21+), and falls back to a cached thread pool on older runtimes.
You can also access the shared instance via `BukkitSpring.getRedisService()`.

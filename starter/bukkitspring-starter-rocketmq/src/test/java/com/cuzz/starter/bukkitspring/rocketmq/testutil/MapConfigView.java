package com.cuzz.starter.bukkitspring.rocketmq.testutil;
    ...
    public ConfigSection getSection(String path) {
        ...
        for (Map.Entry<String, Object> entry : values.entrySet()) {
            ...
            if (key.startsWith(prefix) && key.length() > prefix.length()) {
                String sectionKey = key.substring(prefix.length());
                if (!sectionKey.contains(".")) {
                    sectionValues.put(sectionKey, entry.getValue());
                }
            }
        }
        ...
        return new MapConfigSection(sectionValues);
    }
